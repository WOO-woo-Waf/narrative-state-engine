...
- LLM state analysis: larger semantic chunks around 60K Chinese characters.

Both values are soft budgets, not fixed slice sizes:

```text
NOVEL_AGENT_INGEST_TARGET_CHARS=1600
NOVEL_AGENT_INGEST_OVERLAP_CHARS=180
NOVEL_AGENT_ANALYSIS_TARGET_CHARS=60000
NOVEL_AGENT_ANALYSIS_MAX_CHUNK_CHARS=60000
NOVEL_AGENT_ANALYSIS_CHUNK_OVERLAP_CHARS=0
NOVEL_AGENT_ANALYSIS_PREVIOUS_CONTEXT_CHARS=60000
NOVEL_AGENT_ANALYSIS_JSON_REPAIR_ATTEMPTS=1
NOVEL_AGENT_ANALYSIS_JSON_DEBUG_DIR=logs/analysis_json_failures
NOVEL_AGENT_LLM_MAX_TOKENS=32768
NOVEL_AGENT_LLM_JSON_MAX_TOKENS=32768
...
The 1K ingest value keeps vector evidence focused for recall and reranking. The 60K analysis value is only the target packing budget for grouping paragraphs into a model request.

When JSON parsing fails, the analyzer writes the malformed model output under
`NOVEL_AGENT_ANALYSIS_JSON_DEBUG_DIR`, then asks the model to perform a syntax-only
JSON repair pass before falling back to local partial analysis. This keeps one bad
comma or bracket from discarding a whole chunk.
