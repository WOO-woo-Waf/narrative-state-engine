...
def test_bootstrap_env_ignores_env_local(tmp_path, monkeypatch):
    (tmp_path / ".env").write_text("NOVEL_AGENT_LLM_MODEL=from-env\n", encoding="utf-8")
    (tmp_path / ".env.local").write_text("NOVEL_AGENT_LLM_MODEL=from-local\nNOVEL_AGENT_LOCAL_ONLY=1\n", encoding="utf-8")

    monkeypatch.setattr(config, "PROJECT_ROOT", tmp_path)
    monkeypatch.delenv("NOVEL_AGENT_LLM_MODEL", raising=False)
    monkeypatch.delenv("NOVEL_AGENT_LOCAL_ONLY", raising=False)

    config.bootstrap_env()

    assert os.getenv("NOVEL_AGENT_LLM_MODEL") == "from-env"
    assert os.getenv("NOVEL_AGENT_LOCAL_ONLY") is None
